...
class ProcessGW99DailyBetLog implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $endDate = $date->copy()->endOfDay()->format('Y-m-d H:i:s') . "." . substr(now()->copy()->format('u'), 0, 3);
        $startDate = $date->copy()->startOfDay()->format('Y-m-d H:i:s') . "." . substr(now()->copy()->format('u'), 0, 3);
