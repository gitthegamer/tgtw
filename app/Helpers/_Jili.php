...
use App\Modules\_JiliController;
...
class _Jili
{
    public static function create(Member $member)
    {
        ...
        $response = _JiliController::init("wallet/createMember", [
 ...
 public static function account(Member $member)
    ...
    public static function balance($member)
    {
        ...
        $response = _JiliController::init("wallet/getBalance", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            "alluser" => 0,
    ...
    public static function deposit($member, $transfer)
    {
        ...
        $response = _JiliController::init("wallet/deposit", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
    ...
    public static function withdrawal($member, $transfer)
    {
        ...
        $response = _JiliController::init("wallet/withdraw", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            'userId' => $member_account->username,
            'txCode' => $transfer->uuid,
    ...
    public static function checkTransaction($uuid)
    {
        ...
        $response = _JiliController::init("wallet/checkTransferOperation", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            'txCode' => $transfer->uuid,
        ]);
    ...
    public static function launch(Member $member, $gameid = null, $isMobile = false, $blimit = null)
    {
        ...
        $response = _JiliController::init("wallet/login", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            'userId' => $member_account->username,
            ...
            'platform' => 'JILI',
            'gameType' => 'SLOT',
            'gameForbidden' => '{"PP":{"SLOT":["ALL"],"LIVE":["ALL"]},"FC":{"SLOT":["ALL"]},"SPADE":{"SLOT":["ALL"]},"JDB":{"SLOT":["ALL"]}}',
            'language' => _JiliController::getLocale(),
        ]);
