...
class _Playboy
{
    public static function create(Member $member)
    {
        ...
        $response = _PlayboyController::init("player/create", [
            'PlayerAccount' => config('api.PLAYBOY_PREFIX') . $member->code,
            'Password' => $password = "Abcd" . rand(1000, 9999),
            'Agent' => config('api.PLAYBOY_AGENT'),
        ]);
        ...
        return $member->member_accounts()->updateOrCreate([
            ...
            'account' => $member->code,
            'username' => config('api.PLAYBOY_PREFIX') . $member->code,
            'password' => $password,
        ]);
    }
    ...
    {
        $response = _PlayboyController::init("player/info", [
    ...
    {
        $response = _PlayboyController::init("transaction/create", [
            'PlayerAccount' => $member_account->username,
    ...
    public static function checkTransaction($uuid)
    {
        ...
        $response = _PlayboyController::init("transaction/check", [
            'PlayerAccount' => $member_account->username,
            'ExternalTransactionId' => $transfer->uuid,
        ]);
    ...
    {
        $response = _PlayboyController::init("game/log/agent", [
            'PageNumber' => $page,
