...
class _WBet
{
    public static function create(Member $member)
    {
        ...
        $response = _WBetController::init("createMember.aspx", [
        ...
        $response = _WBetController::init("checkMemberProductUsername.aspx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
    ...
    {
        $response = _WBetController::init("getBalance.aspx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
            'providercode' => config('api.WBET_PROVIDER_CODE'),
    ...
    {
        $response = _WBetController::init("makeTransfer.aspx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
            'providercode' => config('api.WBET_PROVIDER_CODE'),
            'username' => $member_account->account,
            'password' => $member_account->password,
    ...
    public static function checkTransaction($uuid)
    {
        ...
        $response = _WBetController::init("checkTransaction.ashx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
            'referenceid' =>  $transfer->uuid,
        ]);
    ...
    public static function launch(Member $member, $gameid = null, $isMobile = false, $blimit = null)
    {
        ...
        $response = _WBetController::init("launchGames.aspx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
            'providercode' => config('api.WBET_PROVIDER_CODE'),
            'username' => strtolower($member_account->account),
            'password' => $member_account->password,
            'lang' => _WBetController::getLocale(),
            'type' => 'SB',
            'html5' => $isMobile ? '1' : '0',
        ]);
    ...
    {
        $response = _WBetController::init("getGameList.aspx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
            'providercode' => config('api.WBET_PROVIDER_CODE'),
            'reformatJson' => 'yes'
        ]);
    ...
    public static function getBets($version_key = null, $isSkipStore = false)
    {
        if (!$version_key && !$isSkipStore) {
            $version_key = cache()->get('wbet_version_key.' . config('api.WBET_OPERATOR_CODE'), 0);
        }

        $response = _WBetController::init("fetchbykey.aspx", [
            "operatorcode" => config('api.WBET_OPERATOR_CODE'),
        ...
        if (!$isSkipStore) {
            cache()->put(
                'wbet_version_key.' . config('api.WBET_OPERATOR_CODE'),
    ...
    {
        $response = _WBetController::init("checkAgentCredit.aspx", [
            'operatorcode' => config('api.WBET_OPERATOR_CODE'),
        ]);
