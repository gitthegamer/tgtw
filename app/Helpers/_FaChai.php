...
use App\Modules\_FaChaiController;
...
class _FaChai
{
    public static function create(Member $member)
    {
        ...
        $response = _FaChaiController::init("wallet/createMember", [
    ...
    {
        $response = _FaChaiController::init("wallet/getBalance", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            "alluser" => 0,
    ...
    {
        $response = _FaChaiController::init("wallet/deposit", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
    ...
    {
        $response = _FaChaiController::init("wallet/withdraw", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            'userId' => $member_account->username,
            'txCode' => $transfer->uuid,
    ...
    public static function checkTransaction($uuid)
    {
        ...
        $response = _FaChaiController::init("wallet/checkTransferOperation", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            'txCode' => $transfer->uuid,
        ]);
    ...
    public static function launch(Member $member, $gameid = null, $isMobile = false, $blimit = null)
    {
        ...
        $response = _FaChaiController::init("wallet/login", [
            'cert' => config('api.AWC_CERT'),
            'agentId' => config('api.AWC_USER_ID'),
            'userId' => $member_account->username,
            ...
            'platform' => 'FC',
            'gameType' => 'SLOT',
            'gameForbidden' => '{"JILI":{"SLOT":["ALL"]},"PP":{"SLOT":["ALL"],"LIVE":["ALL"]},"SPADE":{"SLOT":["ALL"]},"JDB":{"SLOT":["ALL"]}}',
            'language' => _FaChaiController::getLocale(),
        ]);
