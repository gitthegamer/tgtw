...
class _Lucky365Controller
{
    ...
    {
        $controller = new _Lucky365Controller();
    ...
    {
        return config('api.LK365_LINK') . $function;
    }
    ...
    public function request($function, $params)
    {
        ...
        $method = "POST";
        $logForDB = [
            'channel' => ModelsLog::CHANNEL_L365,
            'function' => $function,
    ...
    public function encypt_string($function)
    {
        if ($function == 'UserInfo/CreatePlayer') {
            return $this->ID . "CreatePlayer" . $this->SN . $this->PlayerCode . config('api.LK365_SECRET_KEY');
        }
        if ($function == 'Account/GetBalance') {
            return $this->ID . "GetBalance" . $this->SN . $this->LoginId . config('api.LK365_SECRET_KEY');
        }
        if ($function == 'Account/SetBalanceTransfer') {
            return $this->ID . "SetBalanceTransfer" . $this->SN . $this->LoginId . config('api.LK365_SECRET_KEY');
        }
        if ($function == 'Account/GetTransferById') {
            return $this->ID . "GetTransferById" . $this->SN . $this->LoginId . $this->RefId . config('api.LK365_SECRET_KEY');
        }
        if ($function == 'Game/GetGameRecord') {
            return $this->ID . "GetGameRecord" . $this->SN . $this->LoginId . config('api.LK365_SECRET_KEY');
        }
        if ($function == 'UserInfo/GetLoginH5') {
            return $this->ID . "GetLoginH5" . $this->SN . $this->LoginId . config('api.LK365_SECRET_KEY');
        }
        if ($function == 'Game/GetGameRecordByTime') {
            return $this->ID . "GetGameRecordByTime" . $this->SN . $this->StartTime . $this->EndTime . config('api.LK365_SECRET_KEY');
        }
    }
