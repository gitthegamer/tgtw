...
class _918kissController
{
    ...
    {
        $controller = new _918kissController();
    ...
    {
        if ($function == "ashx/GameLog.ashx" || $function == "ashx/getOrder.ashx" || $function == "ashx/AgentTotalReport.ashx") {
            return config('api.KISS_LINK_TWO') . $function;
        }
        return config('api.KISS_LINK_ONE') . $function;
    }
        ...
        switch ($function) {
            case "ashx/account/account.ashx?action=RandomUserName":
                return [
                    ...
                    'UserAreaId' => 1,
                    "time" => $time = _918kissController::generateTime(),
                    "authcode" => $this->authcode,
                    "sign" => _918kissController::generateSign($this->authcode, $this->agentName, $time, $this->secretkey),
                ];
            case "ashx/account/account.ashx?action=getUserInfo":
                return [
                    ...
                    "time" => $time = _918kissController::generateTime(),
                    "authcode" => $this->authcode,
                    "sign" => _918kissController::generateSign($this->authcode, $this->userName, $time, $this->secretkey),
                ];
            ...
            case "ashx/account/setScore.ashx":
                return [
                    ...
                    "ActionIp" => _918kissController::generateIP(),
                    "time" => $time = _918kissController::generateTime(),
                    "authcode" => $this->authcode,
                    "sign" => _918kissController::generateSign($this->authcode, $this->userName, $time, $this->secretkey),
                ];
            case "ashx/getOrder.ashx":
                return [
                    "orderid" => $this->orderid,
                    "time" => $time = _918kissController::generateTime(),
                    "authcode" => $this->authcode,
                    "sign" => _918kissController::generateSign($this->authcode, $this->orderid, $time, $this->secretkey),
                ];
    ...
    public function request($function, $params)
    {
        ...
        $method = "POST";
        $logForDB = [
            'channel' => ModelsLog::CHANNEL_918Kiss,
            'function' => $function,
        ...
        if ($function == "ashx/getOrder.ashx") {
            ...
            return [
                'status' => ($response['code'] == '0' || $response['code'] == '1') ? true : false,
                'status_message' => $response['msg'] ?? 'Unknown',
                'data' => $response
            ];
        }
