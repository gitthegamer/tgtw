...
class _SunCityController
{
    ...
    {
        return config('api.SUNCITY_LINK') . $function;
    }

    public static function init($function, $params)
    {
        $controller = new _SunCityController();
    ...
    public function request($function, $params)
    {
        ...
        $method = "POST";
        $logForDB = [
            'channel' => ModelsLog::CHANNEL_SUNCITY,
            'function' => $function,
        ...
        Log::channel($log)->debug("$time Headers: " . json_encode([
            'merchantName' => config('api.SUNCITY_MERCHANT'),
            'Sign' => _SunCityController::generate_signature($params),
    ...
    public static function generate_signature($params)
    {
        ...
        $signature = !empty($new_params) ? urldecode(http_build_query($new_params)) . "&PrivateKey=" . config('api.SUNCITY_API_KEY') : "PrivateKey=" . config('api.SUNCITY_API_KEY');
