...
class _EzugiController
{
    ...
    {
        $controller = new _EzugiController();
    ...
    public function get_url($function)
    {
        if ($function == 'fetchbykey.aspx') {
            return config('api.EZUGI_REPORT_LINK') . $function;
        }
        return config('api.EZUGI_LINK') . $function;
    }
    ...
    public function request($function, $params)
    {
        ...
        $method = "POST";
        $logForDB = [
            'channel' => ModelsLog::CHANNEL_EZUGI,
            'function' => $function,
    ...
    public function encypt_string($function)
    {
        if ($function == "createMember.aspx") {
            return md5($this->operatorcode . $this->username . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "checkMemberProductUsername.aspx") {
            return md5($this->operatorcode . $this->providercode . $this->username . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "getBalance.aspx") {
            return md5($this->operatorcode . $this->password . $this->providercode . $this->username . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "makeTransfer.aspx") {
            return md5($this->amount . $this->operatorcode . $this->password . $this->providercode . $this->referenceid . $this->type . $this->username . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "checkTransaction.ashx") {
            return md5($this->operatorcode . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "getGameList.aspx") {
            return md5($this->operatorcode . $this->providercode . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "launchGames.aspx") {
            return md5($this->operatorcode . $this->password . $this->providercode . $this->type . $this->username . config('api.EZUGI_SECRET_KEY'));
        }
        if ($function == "fetchbykey.aspx") {
            return md5($this->operatorcode . config('api.EZUGI_SECRET_KEY'));
        }
